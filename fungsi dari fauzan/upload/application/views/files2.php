...
if($includeurl)
{
	$includeurl = preg_replace(" ", "", $includeurl);
	if(substr($includeurl, strrpos($includeurl, '/')) != '/') $includeurl.='http://localhost/toms/uploads/';
}
echo $includeurl;
...
<link rel="stylesheet" type="text/css" href="<?php echo $includeurl; ?>assets/dlf/styles.css" />
<?php
if($showthumbnails) {
?>
...
<!--
function o(n, i) {
	document.images['thumb'+n].src = '<?php echo $includeurl; ?>assets/dlf/i.php?f='+i<?php if($memorylimit!==false) echo "+'&ml=".$memorylimit."'"; ?>;

}

function f(n) {
	document.images['thumb'+n].src = 'assets/dlf/trans.gif';
}
//-->
</script>
<?php
...
<div id="container">
  ...
  <div id="listingcontainer">
	...
	if($dirok) {
	?>
	<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode($dotdotdir);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>assets/dlf/dirup.png" alt="Folder" /><strong>..</strong> <em>&nbsp;</em>&nbsp;</a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
	?>
	<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>assets/dlf/folder.png" alt="<?php echo $dirs[$i];?>" /><strong><?php echo $dirs[$i];?></strong> <em>-</em> <?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$dirs[$i]));?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}
	...
	for($i=0;$i<$arsize;$i++) {
		...
		if($showthumbnails && in_array($ext, $supportedimages)) {
			$thumb = '<span><img src="assets/dlf/trans.gif" alt="'.$files[$i].'" name="thumb'.$i.'" /></span>';
	...
	?>
	<div><a href="<?php echo $fileurl;?>" class="<?php echo $class;?>"<?php echo $thumb2;?>><img src="<?php echo $includeurl; ?>assets/dlf/<?php echo $icon;?>" alt="<?php echo $files[$i];?>" /><strong><?php echo $filename;?></strong> <em><?php echo round(filesize($includeurl.$leadon.$files[$i])/1024);?>KB</em> <?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$files[$i]));?><?php echo $thumb;?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}	
